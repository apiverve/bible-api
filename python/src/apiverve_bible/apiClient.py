...
class BibleAPIClientError(Exception):
...
class ValidationError(BibleAPIClientError):
...
class BibleAPIClient:
    ...
    VALIDATION_RULES = {"book": {"type": "string", "required": True}, "chapter": {"type": "integer", "required": True, "min": 1}, "verse": {"type": "integer", "required": False, "min": 1}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the BibleAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises BibleAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[BibleAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns BibleData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises BibleAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise BibleAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise BibleAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise BibleAPIClientError(f"Request failed: {str(e)}")
