...
class BibleValidationException implements Exception {
  final List<String> errors;

  BibleValidationException(this.errors);

  @override
  String toString() => 'BibleValidationException: ${errors.join("; ")}';
}
...
/// Bible API client.
///
/// For more information, visit: https://apiverve.com/marketplace/bible?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [book] (required) - The name of the book in the bible (e.g., Genesis)
/// * [chapter] (required) - The chapter number in the book [min: 1]
/// * [verse] - The verse number in the chapter [min: 1]
class BibleClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'book': ValidationRule(type: 'string', required: true),
    'chapter': ValidationRule(type: 'integer', required: true, min: 1),
    'verse': ValidationRule(type: 'integer', required: false, min: 1),
  };

  BibleClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/bible',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BibleValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BibleValidationException(errors);
    }
  }

  /// Execute a request to the Bible API.
  ///
  /// Parameters are validated before sending the request.
  Future<BibleResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BibleException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BibleResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BibleException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BibleException('Resource not found');
      } else {
        throw BibleException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BibleException) rethrow;
      throw BibleException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Bible API client.
class BibleException implements Exception {
  final String message;

  BibleException(this.message);
